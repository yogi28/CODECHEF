...
int main()
{
	...
	fue(i,1,n)
	{
		while(!s.empty() and a[s.top()]<a[i])
			s.pop();
		if(!s.empty())
			pre[i]=(((i-s.top())*a[i])%MOD+pre[s.top()])%MOD;
		else
			pre[i]=(1ll*i*a[i])%MOD;
		s.push(i);
		if(pre[i]<0)
	...
	fde(i,n,1)
	{
		while(!s.empty() && a[s.top()]>a[i])
			s.pop();
		if(!s.empty())
			suf[i]=(((s.top()-i)*a[i])%MOD+suf[s.top()])%MOD;
		else
			suf[i]=((n-i+1)*a[i])%MOD;
		s.push(i);
		if(suf[i]<0)
	...
	fde(i,n-1,1)
	{
		suf[i]=(suf[i+1]%MOD+suf[i]%MOD)%MOD;
		if(suf[i]<0)
			suf[i]+=MOD;
	}
	ans=0;
